import type { RadarStatus } from "@/types/ui";
...
export function StatusBadge({ status }: { status: RadarStatus }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-sm border px-1.5 py-0.5 text-[10px] font-mono uppercase tracking-wider ${map[status]}`}
    >
      {status}
    </span>
  );
